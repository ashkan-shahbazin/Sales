...
namespace Sales.Persistence.NH.Tests.Integration.QuotasTransactions.Charges.Steps
{
    public class ChargeSteps : BasePersistenceTest
    {
        ...
        public void BuildQuotaTransactionItemForCharge()
        {
            ...
            var quotaTransactionItem = new QuotaTransactionItemTestBuilder().WithActualProductId(_saleActualProduct.Id.DbId)
                .WithProductId(_saleSpecificProduct.Id.DbId).WithItemCode(_saleSpecificProduct.ItemCode).WithCount(5).WithSaleAgent(_saleAgent).WithSegments(segments)
        ...
        {
            Session.BeginTransaction();
            _charge = _chargeTestBuilder.WithQuotaId(_quota.Id).WithReason(Reason.Ui).WithChargeQuotaTransactionDomainService(_chargeTransactionDomainService).BuildCharge();
            _quotaTransactionRepository.Create(_charge);
            Session.Transaction.Commit();
            Session.Clear();
        }
        public void SaveChargeBuildBaseOnSaleDraftOrderRejected()
        {
            Session.BeginTransaction();
            _charge = _chargeTestBuilder.WithQuotaId(_quota.Id).WithReason(Reason.SaleAgentDraftOrderRejected).BuildBaseOnSaleDraftOrderRejected();
            _quotaTransactionRepository.Create(_charge);
            Session.Transaction.Commit();
            Session.Clear();
        }

        public void SavedProperlyOfCharge()
        {
            Session.BeginTransaction();
            var expectedCharge = _quotaTransactionRepository.Get<Charge>(_charge.Id);
            Session.Transaction.Commit();
            Session.Clear();

            Check.That(expectedCharge).IsEqualTo(_charge);
        }
    }
}